...
def main(_):
    ...
    if SHOW_PREDICT:
        ...
        while True:
            try:
                expected.append(sess.run(labels))
            except tf.errors.OutOfRangeError:
                break
        sess.close()
        # Prédiction
        predictions = cnn.predict(input_fn=lambda: input_fn(predict_dataset, None, False, BATCH_SIZE))
        for pred_dict, expec in zip(predictions, expected):
            ...
            letter_pred = LB.inverse_transform(class_id)
            letter_expec = LB.inverse_transform(expec)

            print(template.format(letter_pred, probability * 100, letter_expec))
